...
def peak_hybrid(R, alpha, x0, tau_slow, dt, n_steps, step_time, L_base, dP, eps=1e-6):
    """v3 architecture: exact linear difference first, then a linear-core
    (|x|>=x0 unchanged) / boosted-tail (|x|<x0) shape -- see
    device_hybrid.py's module docstring."""
    def shape(x):
        ax = abs(x)
        if ax >= x0:
            return x
        return np.sign(x) * x0 * (max(ax, eps) / x0) ** alpha
...
def main():
    ...
    peaks_m2 = np.array([peak_mechanism2(5e-3, dt, n_steps, step_time, L_base, dP)
                          for dP in dP_values])
    peaks_hy = np.array([peak_hybrid(1.0, 0.4, 0.006, 5e-3, dt, n_steps, step_time, L_base, dP)
                          for dP in dP_values])
